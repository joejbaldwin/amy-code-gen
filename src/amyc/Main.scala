...
object Main extends MainHelpers:
  ...
  def main(args: Array[String]): Unit =
    ...
    val pipeline =
      AmyLexer.andThen(
        if ctx.printTokens then DisplayTokens
        else Parser.andThen(
          if ctx.printTrees then treePrinterN("Trees after parsing")
          else NameAnalyzer.andThen(
            if ctx.printNames then treePrinterS("Trees after name analysis")
