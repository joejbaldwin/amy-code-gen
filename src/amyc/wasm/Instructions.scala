...
object Instructions:
  ...
  given i2c: Conversion[Instruction, Code] = (i: Instruction) => Code(List(i))
  given is2c: Conversion[List[Instruction], Code] = (is: List[Instruction]) => Code(is)
  given cs2c: Conversion[List[Code], Code] = (cs: List[Code]) => Code(cs flatMap (_.instructions))
end Instructions
