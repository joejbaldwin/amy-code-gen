...
object ModulePrinter:
  ...
  private def mkFun(fh: Function): Document =
    ...
    val exportDoc: Document = if isMain then s"""(export "_start" (func $$$name))""" else ""
    val paramsDoc: Document = if fh.args == 0 then "" else
      Lined(List(
        "(param ",
        Lined(List.fill(fh.args)(Raw("i32")), " "),
        ") "
      ))
    val resultDoc: Document = if isMain then "" else "(result i32) "
